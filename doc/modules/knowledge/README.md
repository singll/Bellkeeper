...
> **定位**：它围绕 `raw / archive / vault` 三层知识模型中的检索增强层展开，但不承担知识真相源职责。
> 当前主链路：**RSSHub → n8n (K02/K01) → Bellkeeper CrawlQueue → 原文落盘 `knowledge/raw` → 筛选提升到 `archive|vault` → Meilisearch 索引 → `!搜`/`!问` 与 Web 检索问答**。
...
- **Layer 2 — Archive**：承接中等价值、可检索的归档资料
- **Layer 3 — Vault**：保留高价值、长期有效、适合进入 PKB 的结构化知识

本模块主要说明 **Archive/Vault + 文件派生检索增强**：

- 如何把外部内容先落地为文件
- 如何基于 `archive + vault` 建立搜索与问答能力
- 如何把自动化加工结果服务于人工整理
...
人工或自动筛选提升
  ...
  └─ vault 高价值卡片
  ↓
TrueNAS `data/knowledge/archive|vault`
  ↓
Meilisearch 文件索引
  ↓
搜索 / 问答层
...
1. **文件优先**：第一落点是 `raw` 原文文件，不是任何向量数据库
2. **索引是派生物**：Meilisearch 索引、问答上下文都从文件派生，可随时重建
3. **问答范围先收敛**：当前阶段只覆盖 `archive + vault`，`raw` 留作可追溯原文，不进入 Meili 索引
...
- 自动爬取与文件级去重、分类、标签、摘要
- 基于 `archive + vault` 的全文检索与 RAG 问答
- Matrix / Web 双入口搜索与问答
- 工作层整理、日报与自动化消费能力
...
- 中等价值、可回溯的归档资料
- 经筛选后适合进入 Meilisearch 的 Markdown
- 保留原文结构，便于搜索、问答和后续重评

### `vault`

- 高价值、长期有效、适合快速复用的知识卡片
- 可与 Obsidian PKB 结构对齐
- 是问答上下文中的高信任层
...
- 标签增强与结构化中间产物
- 日报素材与待沉淀内容
- 当前主要作为 ReportService 和外部输出存档区，不进入知识检索默认索引
...
- `POOL/data/knowledge/archive`
- `POOL/data/knowledge/vault`

`raw` 是入库和追溯层，默认不进入 Meilisearch；`working` 是报告和中间产物区，不作为知识问答默认层。
