...
| 任务 | 状态 | 说明 |
|------|------|------|
| Phase 5: 系统测试与修复 | 🚧 进行中 | 详见 §12 测试报告 |
...
- [x] 设置管理页面完善 (Web UI)
- [x] n8n 工作流集成
  - [x] N8N 配置扩展 (API URL, API Key)
  - [x] WorkflowService 完善 (调用 n8n REST API)
  - [x] 工作流 API (列表、详情、激活/停用、执行历史)
  - [x] Workflows 前端页面
  - [x] Dashboard 快捷操作
...
**立即执行**: Batch 1 修复 — 认证与数据安全 (P0-1, P0-2, P0-3)

修复步骤：
1. 修复 `middleware/auth.go` 中的 Header 名称 (`X-Remote-*` → `Remote-*`)
2. 修复所有 Update handler 的零值覆盖问题（使用 `*bool` 指针）
3. 移除 workflow service 中的 placeholder 数据
4. 种子化默认设置项
5. 部署验证
...
#### P0-1: 认证 Header 名称不匹配 (所有 API 不可用)
...
- **影响范围**: 所有需要认证的 API 端点
- **涉及文件**:
  - `internal/middleware/auth.go:21,46-48` — Header 名称
  - 对应 Header: `Remote-User`, `Remote-Email`, `Remote-Name`, `Remote-Groups`
- **修复方案**: 将中间件的 Header 名称从 `X-Remote-*` 改为 `Remote-*`

#### P0-2: Update 操作零值覆盖导致数据丢失

- **现象**: 编辑数据源/RSS/知识库时，`is_active` 被意外设为 `false`
- **根因**: Go 结构体使用非指针布尔值 `IsActive bool`，JSON 反序列化时未提供的字段默认为 `false`，然后被写入数据库
- **影响范围**: 所有 Update 操作
- **涉及文件**:
  - `internal/handler/datasource.go:20-28,114` — DataSource 更新
  - `internal/handler/rss.go:20-28,117` — RSS 更新
  - `internal/handler/dataset.go:20-29,118` — Dataset 更新
  - `internal/handler/webhook.go:82` — Webhook 更新（直接 BindJSON 到 model）
- **修复方案**: 请求结构体中布尔/数值字段使用指针类型 `*bool`，更新时仅赋值非 nil 字段

#### P0-3: 工作流返回假数据

- **现象**: `/api/workflows/status` 在 n8n 未配置时返回 3 条硬编码的假工作流
- **根因**: `service/workflow.go:86-89,331-337` 中 `getPlaceholderStatus()` 返回虚假数据
- **影响**: 用户看到不存在的工作流，触发会失败；无法区分真假数据
- **修复方案**: 未配置 n8n 时返回空列表 + 提示信息，不返回 placeholder 数据
...
- **根因**: `model/db.go` AutoMigrate 时未种子化默认配置（RagFlow、n8n 等关键配置项）
- **修复方案**: 在初始化时种子化必要的默认设置项（ragflow.api_key, n8n.api_key, n8n.api_url 等）

#### P1-2: Webhook 编辑缺少 headers 字段

- **现象**: 创建/编辑 Webhook 时无法设置自定义 Headers，已有 Headers 在编辑时丢失
- **根因**: `Webhooks.tsx` 表单中缺少 headers 编辑区域
- **涉及文件**: `web/src/pages/Webhooks.tsx:315-410`
- **修复方案**: 在表单中添加 Headers 的 Key-Value 编辑器

#### P1-3: Documents 页面上传验证不足

- **现象**: 未选择知识库时可以提交上传，导致 API 返回不明确的错误
- **根因**: `Documents.tsx:96-125` 上传前未验证 `selectedDataset` 是否已设置
- **修复方案**: 添加前置验证，未选择知识库时禁止上传并提示

#### P1-4: n8n 工作流执行记录不可用

- **现象**: `/api/workflows/executions` 返回 `"n8n API key not configured"`
- **根因**: n8n API Key 需要通过 Settings 配置，但 Settings 为空（关联 P1-1）
- **修复方案**: 配合 P1-1 种子化设置解决；同时改善错误提示
...
────────────────────────────────────────────────────────────────
Batch 1: 认证与数据安全
├── 修复 Auth Header         middleware/auth.go                   P0-1
├── 修复零值覆盖             handler/datasource.go                P0-2
│                            handler/rss.go
│                            handler/dataset.go
│                            handler/webhook.go
└── 移除 Placeholder 数据    service/workflow.go                  P0-3

Batch 2: 设置与配置
├── 种子化默认设置           model/db.go                          P1-1
│                            internal/repository/setting.go
└── n8n 集成修复             (配合 Batch 1)                       P1-4

Batch 3: 前端功能修复
├── Documents 上传验证       web/src/pages/Documents.tsx           P1-3
├── Webhook Headers 编辑     web/src/pages/Webhooks.tsx            P1-2
├── Webhook 触发显示修复     web/src/pages/Webhooks.tsx            P2-1
└── Settings 布尔值修复      web/src/pages/Settings.tsx            P2-2

Batch 4: 体验优化
├── Webhook 历史分页         web/src/pages/Webhooks.tsx            P2-3
├── JSON 错误处理            service/ragflow.go                    P2-4
│                            service/workflow.go
└── 表单验证增强             所有页面                              P3-1
```
...
| `/api/datasources` | CRUD | ⚠️ Update 有问题 | is_active 被零值覆盖 (P0-2) |
| `/api/rss` | CRUD | ⚠️ Update 有问题 | 同上 |
| `/api/webhooks` | CRUD | ⚠️ Update 有问题 | 同上；headers 丢失 (P1-2) |
| `/api/webhooks/:id/trigger` | POST | ✅ 正常 | 触发并记录历史 |
| `/api/webhooks/:id/history` | GET | ✅ 正常 | 返回执行历史 |
| `/api/datasets` | CRUD | ⚠️ Update 有问题 | is_active 被零值覆盖 |
| `/api/settings` | GET/PUT | ⚠️ 空数据 | 无默认设置项 (P1-1) |
| `/api/workflows/status` | GET | ⚠️ 假数据 | 返回 placeholder (P0-3) |
| `/api/workflows/executions` | GET | ❌ 失败 | n8n API key 未配置 (P1-4) |
| `/api/ragflow/documents` | GET | ✅ 正常 | 连接 RagFlow 正常 |
| `/api/ragflow/check-url` | GET | ✅ 正常 | URL 检查正常 |
| 通过 Caddy 认证访问 | ALL | ❌ 全部 401 | Header 不匹配 (P0-1) |

---

**文档状态**: 测试与修复中

**当前阶段**: Phase 5 - 系统测试与修复
