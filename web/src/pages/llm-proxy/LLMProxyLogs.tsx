...
interface LLMProxyLogsProps {
  channelConfigs: LLMChannelConfig[] | undefined
}

const LLMProxyLogs: Component<LLMProxyLogsProps> = (props) => {
                ...
                <For each={logs()}>
                  {(log) => (
                    <tr>
                      ...
                      <td>
                        <span class={`badge ${statusCodeBadge(log.status_code)}`}>
                          {log.status_code || '--'}
                        </span>
                      </td>
                      <td class={`text-sm ${durationClass(log.duration_ms)}`}>
                        {log.duration_ms > 0 ? `${log.duration_ms}ms` : '--'}
                      </td>
                      <td class="text-sm text-dark-300">
                        {log.prompt_tokens > 0 ? log.prompt_tokens : '--'}
                      </td>
                      <td class="text-sm text-dark-300">
                        {log.comp_tokens > 0 ? log.comp_tokens : '--'}
                      </td>
                      <td class="text-xs text-dark-400">{log.caller_id || '--'}</td>
                      <td class="text-xs text-red-400 max-w-[200px] truncate" title={log.error_message}>
                        {log.error_message || '--'}
                      </td>
                    </tr>
                  )}
